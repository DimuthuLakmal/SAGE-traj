...
from tbsim.utils.internvl_utils import get_pixel_values
...
def resize_and_draw_points(
    ...
    return_chw: bool = True,
    resize = False
):
    """
    img_chw: numpy array of shape (3, H, W)
    points: iterable of (x, y) points in the resized 56x56 image space
    radius: circle radius
    color: RGB tuple
    thickness: -1 means filled circle
    interpolation: cv2 interpolation method
    return_chw: if True returns shape (3, 56, 56), else (56, 56, 3)
    """
    ...
    # Ensure OpenCV-compatible dtype
    ...
    if resize:
        final_image = cv2.resize(img_hwc, (56, 56), interpolation=interpolation)
    else:
        final_image = img_hwc
    ...
    for i, (x, y) in enumerate(points):
        if i == 0:
            cv2.circle(final_image, (int(x), int(y)), radius, (255,0,0), thickness)
        else:
            cv2.circle(final_image, (int(x), int(y)), radius, (0,255,0), thickness)
...
def generate_pixel_values(data, maps, resize=False, llm_config=None):
    all_b_data = []
    for b_data in data:
        all_t_data = []
        for t in range(0, 4):
            ...
            for i in range (len(b_data)):
                if resize:
                    t_data.append((int(b_data[i][t*2 + 1][0]/4), int(b_data[i][t*2 + 1][1]/4)))
                else:
                    t_data.append((int(b_data[i][t*2 + 1][0]), int(b_data[i][t*2 + 1][1])))

            all_t_data.append(t_data)
        all_b_data.append(all_t_data)

    pixel_values_b = []
    maps_np = maps.detach().cpu().numpy().copy()
    for b, b_data in enumerate(all_b_data):
        images_t = []
        for t in range(4):
            result = resize_and_draw_points(maps_np[b], b_data[t], resize=resize)
            result = Image.fromarray(result)
            images_t.append(result)

        pixel_values = get_pixel_values(images_t, llm_config)
        pixel_values_b.append(pixel_values)

    return pixel_values_b
...
def rasterize_agents(
    ...
    for b_data in raster_hist_pos.permute(0,2,1,3):
        all_n_data_str = ''
        all_n_data = []
        for n_data in b_data:
            all_t_data_str = ''
            all_t_data = []
            for i, t_data in enumerate(n_data):
                if i%4 == 0:
                    all_t_data_str += (str(t_data[0].item()) + "," + str(t_data[1].item()) + ",")
                    all_t_data.append((t_data[0].item(), t_data[1].item()))

            all_t_data_str = all_t_data_str[:-1] + "|"
            all_n_data_str += all_t_data_str
            all_n_data.append(all_t_data)

        all_n_data_str = 't4:<image>\n t8:<image>\n t12:<image>\n t16:<image>\nx1,y1,x2,y2,x3,y3,x4,y4,x5,y5,x6,y6,x7,y7,x8,y8|' + all_n_data_str
        all_batch_data_str.append(all_n_data_str)
        all_batch_data.append(all_n_data)

    llm_images = generate_pixel_values(all_batch_data, maps, resize=False, llm_config=llm_config)
    # llm_images = None
    # all_batch_data_str = None
    ...
    maps_hist_.scatter_(dim=2, index=raster_hist_pos_flat[:, :, 1:].unsqueeze(dim=-1).repeat(1, 1, 1, 3),
                        src=torch.ones((b, t, h * w, 1), device=maps.device) * white)  # mark other agents with -1
    maps_hist_.scatter_(dim=2, index=raster_hist_pos_flat[:, :, [0]].unsqueeze(dim=-1).repeat(1, 1, 1, 3),
                        src=torch.ones((b, t, h * w, 1), device=maps.device) * green)  # mark ego with 1.
    ...
    maps = torch.cat((hist_image, maps), dim=1) 


    raster_hist_pos[..., 0] = raster_hist_pos[..., 0] / (w-1)
    raster_hist_pos[..., 1] = raster_hist_pos[..., 1] / (h-1)

    return maps, hist_image_, relative_dist, agent_hist_pos, all_batch_data, llm_images, all_batch_data_str
...
def convert_neigh_to_agent_relative(agent_hist_pos, raster_from_agent, agent_mask, w, h):
    b,a,t,_ = agent_hist_pos.shape
    ...
    return raster_hist_pos


def parse_node_centric(batch: dict, overwrite_nan=True, reset=False, llm_config=None):
    ...
    # neigh_indices = neigh_indices[:, 2:]
    # batch["neigh_hist"] = batch["neigh_hist"][:, 2:, :, :]
    ...
    B, _, h, w = maps_rasterize_in.shape
    neigh_fut_pos_rel = convert_neigh_to_agent_relative(neigh_fut_pos, raster_from_agent, neigh_fut_mask, w, h)

    scene_index = torch.arange(B).unsqueeze(-1)
    ...
    if BATCH_RASTER_CFG["include_hist"]:
        maps, hist_maps, relative_xy, all_hist_pos, all_batch_data, llm_images, llm_text = rasterize_agents(
            # batch['llm_input_ids'],
            maps_rasterize_in,
            all_hist_pos,
            all_hist_yaw,
    ...
    d = dict(
        ...
        llm_text=llm_text,
        neigh_fut_pos_rel=neigh_fut_pos_rel,
...
@torch.no_grad()
def parse_trajdata_batch(batch: dict, overwrite_nan=True, reset=False, llm_config=None):
    ...
    if "num_agents" in batch:
        ...
        # agent centric
        d = parse_node_centric(batch, overwrite_nan=overwrite_nan, reset=reset, llm_config=llm_config)
...
def extract_data_batch_for_guidance_pure_diff(data_batch, mode='testing', data_cache_dir=None):
    data_batch_for_guidance = {}
    if mode == 'testing':
        guidance_keywords_used = guidance_keywords
    elif mode == 'training':
    ...
    for k in guidance_keywords_used:
        if k == 'extras':
            if 'extras' in data_batch:
                data_batch_for_guidance['extras'] = {}
                for j in data_batch['extras']:
                    data_batch['extras'][j] = data_batch['extras'][j].unsqueeze(dim=1)
                    B, M = data_batch['extras'][j].shape[:2]
                    data_batch_for_guidance['extras'][j] = data_batch['extras'][j].reshape(B*M, *data_batch['extras'][j].shape[2:])
        elif k == 'map_names':
            if isinstance(data_batch[k][0], list):
                ...
                map_name = data_batch[k][0]
            vec_map = load_vec_map(map_name, cache_path=data_cache_dir)
            data_batch_for_guidance['vec_map'] = vec_map
        else:
            if k == 'scene_index' and k not in data_batch and 'scene_ids' in data_batch:
                k2 = 'scene_ids'
                ...
                k2 = k
                data_batch[k2] = data_batch[k2].unsqueeze(dim=1)
                B, M = data_batch[k2].shape[:2]
                data_batch_for_guidance[k] = data_batch[k2].reshape(B*M, *data_batch[k2].shape[2:])
