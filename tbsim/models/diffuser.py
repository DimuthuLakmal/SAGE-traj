...
from .st_unet import STUNetMapEncoder
...
class DiffuserModel(nn.Module):
    ...
    def __init__(
        ...
        if self.use_conditioning and (self.rasterized_map or self.rasterized_hist):
            ...
            self.map_encoder = STUNetMapEncoder(
                image_channels=3,
        ...
        self.llm_config = {
            ...
            'input_size': 224,
            'use_thumbnail': llm_model.config.use_thumbnail,
            'max_num': 1,
            'max_new_tokens': 8192,
            'num_beams': 1
        }
        self.transformer = SpatioTemporalTransformer(encoder_config, decoder_config, llm_model, llm_tokenizer, self.llm_config)
        ...
        if diffuser_model_arch == "TemporalMapUnet":
            ...
            self.model = TemporalMapUnet(horizon=horizon,
                                      ...
                                      cond_dim=cond_out_feat_size+llm_dim,
    ...
    def get_aux_info(self, data_batch, plan=None, include_class_free_cond=False):
        ...
        if self.map_encoder is not None:
            ...
            map_global_feat, map_grid_feat = self.map_encoder(image_hist_batch)
        ...
        aux_info = {
            ...
            'llm_images': data_batch['llm_images'],
            'llm_text': data_batch['llm_text']
        }
        # if include_class_free_cond:
    ...
    def get_condition_gates(self, t):
        # t: (B,)
        tau = t.float() / float(self.n_timesteps - 1)  # high tau = noisy step

        # semantic high at noisy steps, numerical high at clean steps
        k = 10.0
        c = 0.5
        w_sem = torch.sigmoid(k * (tau - c))
        w_num = 1.0 - w_sem

        return w_sem[:, None], w_num[:, None]

    def compute_losses(self, data_batch):
        batch_size = data_batch['agent_fut'].shape[0]
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=data_batch['agent_fut'].device).long()
        w_sem, w_num = self.get_condition_gates(float(t.item()))

        aux_info = self.get_aux_info(data_batch)
        sem_cond = self.transformer(aux_info)
        
        cond_feat = torch.cat([w_sem * sem_cond, w_num * aux_info["cond_feat"]], dim=-1)
        ...
        diffusion_loss, _ = self.loss(x, data_batch, aux_info=aux_info, t=t)
        losses = OrderedDict(
    ...
    def p_sample_loop(self, shape, data_batch, num_samp, 
        ...
        aux_info_no_llm_data = {
            'cond_feat': aux_info['cond_feat'],
            'curr_states': aux_info['curr_states'],
            'all_hist_feat': aux_info['all_hist_feat']
        }
        aux_info_no_llm_data = TensorUtils.repeat_by_expand_at(aux_info_no_llm_data, repeats=num_samp, dim=0)
        ...
        sem_cond = self.transformer(aux_info)
        sem_cond = TensorUtils.repeat_by_expand_at(sem_cond, repeats=num_samp, dim=0)
        ...
        for i in steps:
            ...
            w_sem, w_num = self.get_condition_gates(timesteps)
            cond_feat = torch.cat([w_sem * sem_cond, w_num * aux_info_no_llm_data["cond_feat"]], dim=-1)
            aux_info_no_i = {
                'cond_feat': cond_feat,
                'curr_states': aux_info_no_llm_data['curr_states'],
                'all_hist_feat': aux_info_no_llm_data['all_hist_feat']
            }
            
            x, guide_losses = self.p_sample(x, timesteps, data_batch, aux_info=aux_info_no_i, num_samp=num_samp, class_free_guide_w=class_free_guide_w,
    ...
    def loss(self, x, data_batch, aux_info={}, t=0):
        batch_size = len(x)
