...
import transformers
from tbsim.models.internvl.dist_utils import init_dist
from tbsim.models.internvl.model.internlm2.modeling_internlm2 import InternLM2ForCausalLM
from tbsim.models.internvl.model.internvl_chat import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModel)
from tbsim.models.internvl.patch import (concat_pad_data_collator,
                            dpo_concat_pad_data_collator,
                            replace_llama_rmsnorm_with_fused_rmsnorm,
                            replace_train_sampler)
from tbsim.models.internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      IMG_CONTEXT_TOKEN, IMG_END_TOKEN,
                                      IMG_START_TOKEN, QUAD_END_TOKEN,
                                      QUAD_START_TOKEN, REF_END_TOKEN,
                                      REF_START_TOKEN)
from tbsim.models.internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    WeightedConcatDataset, build_transform,
                                    dynamic_preprocess, preprocess,
                                    preprocess_internlm,
                                    preprocess_internvl2_5, preprocess_mpt,
                                    preprocess_phi3)
from tbsim.models.internvl.train.trainer_dpo import MultimodalDPOTrainer
...
class ModelArguments:
    ...
    use_4bit: bool = field(
        default=True,
        metadata={'help': 'Enable 4-bit quantized loading for QLoRA.'},
    )
    use_8bit: bool = field(
        default=False,
        metadata={'help': 'Enable 8-bit quantized loading (less compression than 4-bit).'},
    )
    bnb_4bit_quant_type: str = field(
        default="nf4",
        metadata={'help': '4-bit quant type: nf4 or fp4 (nf4 recommended).'},
    )
    bnb_4bit_use_double_quant: bool = field(
        default=False,
        metadata={'help': 'Use double quantization (usually helps memory/quality).'},
    )
...
def find_lora_targets(model, wanted_suffixes, max_print=200):
    import re
    ...
    print(f"[LoRA] Found {len(hits)} Linear modules matching suffixes.")
    for n in hits[:max_print]:
        print("  ", n)
    return hits


def save_adapters_only(mm_model, tokenizer, output_dir): 
    ...
    # LLM LoRA
    if hasattr(mm_model, "language_model") and hasattr(mm_model.language_model, "save_pretrained"):
        mm_model.language_model.save_pretrained(os.path.join(output_dir, "llm_lora"))

    # Vision LoRA (if you used it)
    if hasattr(mm_model, "vision_model") and hasattr(mm_model.vision_model, "save_pretrained"):
        mm_model.vision_model.save_pretrained(os.path.join(output_dir, "vision_lora"))

    # Projector (if you trained it, otherwise skip)
    torch.save(mm_model.mlp1.state_dict(), os.path.join(output_dir, "mlp1.pt"))
...
class SaveAdaptersCallback(TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        ...
        if hasattr(model, "language_model") and hasattr(model.language_model, "save_pretrained"):
            model.language_model.save_pretrained(os.path.join(ckpt_dir, "llm_lora"))

        # Save Vision LoRA (if you trained it)
        if hasattr(model, "vision_model") and hasattr(model.vision_model, "save_pretrained"):
            model.vision_model.save_pretrained(os.path.join(ckpt_dir, "vision_lora"))

        # Save projector if trainable
        if hasattr(model, "mlp1"):
            torch.save(model.mlp1.state_dict(), os.path.join(ckpt_dir, "mlp1.pt"))

        # Save tokenizer once (optional, but cheap)
        if tokenizer is not None:
            tokenizer.save_pretrained(ckpt_dir)

        print(f"[Checkpoint] Saved adapters at {ckpt_dir}")
        return control
...
def main():
    ...
    if model_args.use_liger:
        from tbsim.models.internvl.patch import apply_liger_kernel_to_internvit
    ...
    if model_args.model_name_or_path is not None:
        ...
        if model_args.use_4bit or model_args.use_8bit:
            quantization_config = BitsAndBytesConfig(
                ...
                bnb_4bit_compute_dtype=torch.bfloat16,   # good default on A100/H100, etc.
            )

        model = InternVLChatModel.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config, cache_dir="/data/gpfs/projects/punim2030/huggingface/cache",
    ...
    if model.config.vision_config.image_size != data_args.force_image_size:
        ...
        params = [model.vision_model.embeddings.position_embedding]
    ...
    if quantization_config is not None:
        # only the LLM needs k-bit training prep; keep vision as-is
        model.language_model = prepare_model_for_kbit_training(
            model.language_model,
            use_gradient_checkpointing=model_args.grad_checkpoint,
        )
    ...
    wanted_suffixes = ['self_attn.q_proj', 'self_attn.k_proj', 'self_attn.v_proj', 'self_attn.o_proj',
                              'mlp.gate_proj', 'mlp.down_proj', 'mlp.up_proj','attn.qkv', 'attn.proj', 'mlp.fc1', 'mlp.fc2']
    target_modules = find_lora_targets(model, wanted_suffixes)
    ...
    trainer = MultimodalDPOTrainer(
        ...
        callbacks=[SaveAdaptersCallback()]
    )
    ...
    if training_args.do_train:
        ...
        save_adapters_only(model, tokenizer, training_args.output_dir)
